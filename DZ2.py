...
class CoffeeShop:
    ...
    def serve_coffee(self, coffee_type):
        if coffee_type in self.menu:
            print(f"Serving {coffee_type}. Enjoy your coffee!")
            return self.menu[coffee_type]
        else:
            print(f"Sorry, we don't have {coffee_type} on the menu.")
            return 0
...
class Human:
    ...
    def live(self, day, coffee_shop):
        ...
        self.drink_coffee(coffee_shop)

    def drink_coffee(self, coffee_shop):
        coffee_type = random.choice(list(coffee_shop.menu.keys()))
        cost = coffee_shop.serve_coffee(coffee_type)

        if cost > 0:
            print(f"Bought {coffee_type} for {cost} money.")
            self.money -= cost
            self.gladness += 5
            self.satiety += 2
...
for day in range(1, 8):
    if nick.live(day, cafe) is False:
        break
